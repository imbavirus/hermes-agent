...
/** The sessions list is a switcher, not a chat strip. A tile that names it as
 *  the dock anchor would stack as a tab over the session names. Send those
 *  to workspace (the focused chat zone still wins when it is a real strip). */
export function sessionTileDockAnchor(anchor: null | string | undefined): string | undefined {
  if (anchor === 'sessions') {
    return 'workspace'
  }
...
export function openSessionTile(
  ...
  const dock = sessionTileDockAnchor(anchor ?? focusedSessionTabAnchor())
