...
describe('parseDeepLinkSessionAction', () => {
  ...
  it('parses session/open path id', () => {
    expect(
      parseDeepLinkSessionAction({ kind: 'session', name: 'open/abc123', params: {} })
    ).toEqual({ action: 'open', storedSessionId: 'abc123' })
  })

  it('parses session/open query id', () => {
    expect(
      parseDeepLinkSessionAction({ kind: 'session', name: 'open', params: { id: 'xyz' } })
    ).toEqual({ action: 'open', storedSessionId: 'xyz' })
  })
