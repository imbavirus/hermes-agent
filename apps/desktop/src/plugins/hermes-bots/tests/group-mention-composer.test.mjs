...
test('group room composers mount GroupMentionInput, not bare Input', () => {
  // One bottom composer (reply by default, New Thread when composing).
  assert.match(source, /jsx\(GroupMentionInput, \{/)
  assert.match(source, /'aria-label': composingNew \? `New thread in \$\{group\}` : `Reply in \$\{group\}`/)
